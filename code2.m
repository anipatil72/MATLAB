...
% Monnna :  (1+x)*y''(x)+1*y'(x)+0*y(x)=-x^2        y(0)=0     y(1)=0
%
% ======Start Command Window======
% syms x;
% [approx,exac]=odegalerkin([1 0 3 x^2+2],[2 4 0 7],25)
  syms x
 [approx,exac]=odegalerkin([1 + x 1 0 -x^2],[0 0.000000001 1 0.0000000001],25)
...
elseif length(bc)==2
    ...
    error('The size of the boundary conditions matrix is not valid.')
end
phider1=diff(phi);
